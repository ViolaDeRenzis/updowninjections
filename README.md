...
- The `Backpropagation_*.dat` files contain the posterior samples of the tilt angles $\theta_1$ and $\theta_2$ that are numerically evolved from $20$ Hz to $0$ Hz. The posterior distributions for the tilt angles at $20$ Hz are contained in the `*.json` files.
